...
def test_build_from_subscriptions_duck_typed():
    # 宿主订阅行的媒体身份是单一 (media_source, media_id) 二元组。
    subs = [
        SimpleNamespace(media_source="themoviedb", media_id="550", type="电影", season=None),
        SimpleNamespace(media_source="themoviedb", media_id="1396", type="电视剧", season=3),
        SimpleNamespace(media_source="douban", media_id="D42", type="电影", season=None),
    ]
...
def test_build_skips_unmappable_media_source():
    # 无对应强标识槽位的来源不入索引，交由 exists() 兜底，避免假阳性命中。
    subs = [SimpleNamespace(media_source="imdb", media_id="tt0137523", type="电影", season=None)]
    idx = build_subscribed_index(subs)
    assert idx.contains(RankMediaItem(title="a", tmdb_id=550, type_hint=MediaType.MOVIE)) is False


def test_build_tolerates_bad_rows():
    # 单条异常不应影响整体构建
    bad = SimpleNamespace()  # 缺全部属性
    good = SimpleNamespace(media_source="themoviedb", media_id="550", type="电影", season=None)
