...
用 MagicMock 造 ProviderContext 的三条链，真实 HistoryStore（内存库）断言历史与去重集合。
"""
...
def _mediainfo(mtype=MediaType.MOVIE, tmdb_id=438631, douban_id="123",
    ...
    # 宿主按 (media_source, media_id) 定位媒体：识别结果的主身份，订阅时取用的就是它。
    mi.media_source = MediaSource.TMDB
    mi.media_id = str(tmdb_id)
    mi.year = year
    mi.season = season
...
def test_filtered_post():
    ...
    # douban 识别分支
    recognize_kwargs = ctx.chain.recognize_media.call_args.kwargs
    assert recognize_kwargs.get("media_source") == MediaSource.Douban
    assert recognize_kwargs.get("media_id") == "123"
    assert store.query()[0][0]["status"] == "filtered"
...
def test_subscription_exists():
    ...
    recognize_kwargs = ctx.chain.recognize_media.call_args.kwargs
    assert recognize_kwargs.get("media_source") == MediaSource.Bangumi
    assert recognize_kwargs.get("media_id") == "42"
...
def test_subscribed_movie():
    ...
    # tmdb 识别分支
    recognize_kwargs = ctx.chain.recognize_media.call_args.kwargs
    assert recognize_kwargs.get("media_source") == MediaSource.TMDB
    assert recognize_kwargs.get("media_id") == "438631"
    # add 关键参数
    ...
    # 媒体身份取自识别结果的主身份，而非榜单条目自带的 ID
    assert add_kwargs["media_source"] == MediaSource.TMDB
    assert add_kwargs["media_id"] == "438631"
