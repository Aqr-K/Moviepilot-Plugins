...
# 服务端统计项以单一 (media_source, media_id) 表达媒体身份，一条只带一种来源的 ID。
def _stat_movie():
    return {"type": "movie", "name": "电影X", "media_source": "themoviedb", "media_id": "100",
            "year": 2021, "count": 500, "vote": 8.1, "poster": "pm.jpg", "season": None}


def _stat_tv():
    return {"type": "tv", "name": "剧Y", "media_source": "bangumi", "media_id": "10",
            "year": 2020, "count": 300, "season": 1, "poster": "pt.jpg"}


def _stat_unknown():
    return {"type": "weird", "name": "未知Z", "media_source": "themoviedb", "media_id": "300"}
...
def test_popular_fetch_maps_movie_and_tv(monkeypatch):
    ...
    # stype 按服务端要求传中文，不是 movie/tv。
    assert calls == [(MediaType.MOVIE.value, 5, None, None), (MediaType.TV.value, 7, None, None)]
    ...
    assert movie.unique_seed == "电影X:themoviedb:100"
    # 身份只有一个来源，其余槽位留空
    assert movie.douban_id is None and movie.bangumi_id is None

    tv = by_cat["tv"]
    assert tv.type_hint == MediaType.TV
    assert "anime_mode" not in tv.source_meta
    assert tv.bangumi_id == 10             # media_source=bangumi -> 落 bangumi 槽位
    assert tv.season == 1
    assert tv.tmdb_id is None
    assert tv.douban_id is None
    assert tv.tvdb_id is None


def test_popular_fetch_genre_fanout(monkeypatch):
    """电影多选风格 -> 逐 genre 分别请求，跨 genre 按媒体身份去重合并。"""
    calls = []

    def _fake(stype, page=1, count=30, genre_id=None, min_rating=None, **kw):
        calls.append((stype, genre_id))
        if genre_id == 16:
            return [_stat_movie(), {"type": "movie", "name": "动画A",
                                    "media_source": "themoviedb", "media_id": "101"}]
        if genre_id == 878:
            return [_stat_movie(), {"type": "movie", "name": "科幻B",
                                    "media_source": "themoviedb", "media_id": "102"}]
        return []
    ...
    # 逐 genre 各请求一次
    assert calls == [(MediaType.MOVIE.value, 16), (MediaType.MOVIE.value, 878)]
    # 电影X(100) 在两个 genre 里都出现 -> 去重后只一条；共 3 条(100/101/102)
    assert len(items) == 3
    assert {i.unique_seed for i in items} == {
        "电影X:themoviedb:100", "动画A:themoviedb:101", "科幻B:themoviedb:102"}
    # genre_id 记入 source_meta（保留首次命中的 16）
    x = next(i for i in items if i.unique_seed == "电影X:themoviedb:100")
...
def test_popular_popularity_filter(monkeypatch):
    ...
    def _fake(stype, page=1, count=30, genre_id=None, min_rating=None, **kw):
        ...
        return [_stat_movie(), {"type": "movie", "name": "冷门M", "media_source": "themoviedb",
                                "media_id": "199", "count": 100}]
    ...
    assert {i.unique_seed for i in items} == {"电影X:themoviedb:100"}
